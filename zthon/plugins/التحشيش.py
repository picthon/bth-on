...
async def _(zed):
    await edit_or_reply (zed, "𓆰 [𝙎𝙊𝙐𝙍𝘾𝞝 𝗕𝗧𝗛𝗢𝗡 - 𝘿𝙀𝙇𝙀𝙏𝙀](t.me/BThon) 🗑♻️𓆪\n**𓍹━─━─━─━─𝐁𝐓𝐇𝐎𝐍─━─━─━─━𓍻**\n\n **✵│رابـط الحـذف ↬** https://telegram.org/deactivate \n\n\n **✵│بـوت الحـذف  ↬** @LC6BOT ")
...
async def permalink(mention):
    ...
    if not user:
        return
    if user.id == 5448642653 or user.id == 5448642653 or user.id == 5448642653:
...
ZelzalTHS_cmd = (
    "𓆩 [𝗦𝗼𝘂𝗿𝗰𝗲 𝗕𝗧𝗛𝗢𝗡 𝗖𝗼𝗻𝗳𝗶𝗴 𝗩𝗮𝗿𝘀 - اوامـر التحشيش](t.me/BThon) 𓆪\n\n"
...
async def permalink(mention):
    ...
    if not user:
        return
    if user.id == 5448642653 or user.id == 5448642653 or user.id == 5448642653:
        return await edit_or_reply(mention, f"**╮ ❐ لك هـذا بـلاك . . شما حجيت ماكـدر اوصفـه 🙊💘 ❏╰**")
    tag = user.first_name.replace("\u2060", "") if user.first_name else user.username
