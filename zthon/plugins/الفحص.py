...
async def amireallyalive(event):
    ...
    ALIVE_TEXT = gvarstatus("ALIVE_TEXT") or "** بـوت ببيـثون 𝐁𝐓𝐇𝐎𝐍 يعمـل .. بنجـاح ☑️ 𓆩 **"
    ZED_IMG = gvarstatus("ALIVE_PIC") or "https://graph.org/file/3594dbbd4e44a846ec361.mp4"
...
async def amireallyialive(event):
    ...
    reply_to_id = await reply_id(event)
    Z_EMOJI = gvarstatus("ALIVE_EMOJI") or "✥┊"
    zed_caption = "** بـوت ببيـثون 𝐁𝐓𝐇𝐎𝐍 يعمـل .. بنجـاح ☑️ 𓆩 **\n"
    zed_caption += f"**{Z_EMOJI} إصـدار التـيليثون :** `{version.__version__}\n`"
    zed_caption += f"**{Z_EMOJI} إصـدار ببيـثون :** `{repversion}`\n"
